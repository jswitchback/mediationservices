...
function sitename_zen_library() {
  ...
  $libraries['modernizr'] = array(
    ...
    'version' => '1.0',
    'js' => array(
      drupal_get_path('theme', 'sitename_zen') . '/js/lib-vendor/modernizr.dev.js' => array(
        'type' => 'file',
        'weight' => 1000,
        'group' => JS_LIBRARY),
    ),
  ...
  $libraries['matchMedia_polyfill'] = array(
    ...
    'version' => '1.0',
    'js' => array(
      drupal_get_path('theme', 'sitename_zen') . '/js/lib-vendor/media.match.min.js' => array(
        'type' => 'file',
        'weight' => 1002,
        'group' => JS_LIBRARY),
    ),
  ...
  $libraries['enquire'] = array(
    ...
    'version' => '1.0',
    'js' => array(
      drupal_get_path('theme', 'sitename_zen') . '/js/lib-vendor/enquire.min.js' => array(
        'type' => 'file',
        'weight' => 1003,
        'group' => JS_LIBRARY),
    ),
  ...
  $libraries['to_top'] = array(
    ...
    'version' => '1.0',
    'js' => array(
      drupal_get_path('theme', 'sitename_zen') . '/js/lib-conditional/to-top-button.js' => array(
        'type' => 'file',
        'group' => JS_THEME),
    ),
    'css' => array(
      drupal_get_path('theme', 'sitename_zen') . '/css/lib-conditional/to-top-button.css' => array(
        'type' => 'file',
        'media' => 'screen',
      ),
    ),
  ...
  $libraries['responsive_js'] = array(
    ...
    'version' => '1.0',
    'js' => array(
      drupal_get_path('theme', 'sitename_zen') . '/js/lib-base/responsive-app.js' => array(
        'type' => 'file',
        'weight' => 1004,
        'group' => JS_LIBRARY),
    ),
  ...
  $libraries['touch'] = array(
    ...
    'version' => '1.0',
    'js' => array(
      drupal_get_path('theme', 'sitename_zen') . '/js/lib-conditional/touch.js' => array(
        'type' => 'file',
        'weight' => 1005,
        'group' => JS_LIBRARY),
    ),
    'css' => array(
      drupal_get_path('theme', 'sitename_zen') . '/css/lib-conditional/touch.css' => array(),
    ),
  );
 ...
 *   also be "maintenance_page" since sitename_zen_preprocess_maintenance_page() calls
 *   this function to have consistent variables.
 */
function sitename_zen_preprocess_html(&$variables, $hook) {
  // Add variables and paths needed for theme settings in html.tpl
  $variables['base_path'] = base_path();
  $variables['path_to_sitename_zen'] = drupal_get_path('theme', 'sitename_zen');

  $variables['to_top'] = theme_get_setting('sitename_zen_to_top');
  $to_top = $variables['to_top'];

  $variables['app_icons'] = theme_get_setting('sitename_zen_app_icons');
  $app_icons = $variables['app_icons'];

  $variables['ms_tile_color'] = theme_get_setting('sitename_zen_ms_tile_color');
  $ms_tile_color = $variables['ms_tile_color'];

  $variables['accordion_menu'] = theme_get_setting('sitename_zen_accordion_menu');
  $accordion_menu = $variables['accordion_menu'];

  $variables['compass_grid'] = theme_get_setting('sitename_zen_compass_grid');
  $compass_grid = $variables['compass_grid'];

  $viewport_indicator = theme_get_setting('sitename_zen_browser_width_indicator');

  // Add CSS/JS based on theme settings
  if ($to_top) {
    drupal_add_js(drupal_get_path('theme', 'sitename_zen') . '/js/lib-vendor/jquery.smooth-scroll.js',  array('type' => 'file','weight' => 1045,'group' => JS_LIBRARY));
    drupal_add_library('sitename_zen', 'to_top');
  }

  if ($compass_grid && $variables['is_admin'] ) {
      $variables['attributes_array']['data-development-grid'][] = 'hide';
      drupal_add_js(drupal_get_path('theme', 'sitename_zen') . '/js/lib-conditional/grid.js', array('group' => JS_THEME, 'weight' => -10, 'every_page' => TRUE));
      drupal_add_css(drupal_get_path('theme', 'sitename_zen') . '/css/lib-conditional/grid.css', array('group' => CSS_THEME, 'weight' => -10, 'every_page' => TRUE));
  }

  if ($viewport_indicator) {
    if ($variables['is_admin'] && !module_exists('overlay')) {
      $variables['attributes_array']['class'][] = 'sitename_zen_browser-width-indicator';
      drupal_add_js(drupal_get_path('theme', 'sitename_zen') . '/js/lib-conditional/viewport-indicator.js', array('group' => JS_THEME, 'weight' => -10, 'every_page' => TRUE));
      drupal_add_css(drupal_get_path('theme', 'sitename_zen') . '/css/lib-conditional/viewport-indicator.css', array('group' => CSS_THEME, 'weight' => -10, 'every_page' => TRUE));
    }
  }

  if (!empty($variables['page']['theming_sidebar']) && $variables['is_admin']) {
    $variables['classes_array'][] = 'has-theming-sidebar';
    drupal_add_js(drupal_get_path('theme', 'sitename_zen') . '/js/lib-conditional/theming-sidebar.js', array('type' => 'file'));
    drupal_add_css(drupal_get_path('theme','sitename_zen') . '/css/lib-conditional/theming-sidebar.css');
  }
  ...
  if (!empty($variables['page']['offcanvas_sidebar'])) {
    ...
    drupal_add_js(drupal_get_path('theme', 'sitename_zen') . '/js/lib-conditional/offcanvas-sidebar.js', array('type' => 'file','weight' => 1050,'group' => JS_LIBRARY));
    drupal_add_css(drupal_get_path('theme','sitename_zen') . '/css/lib-conditional/offcanvas-sidebar.css');
  }

  if ($accordion_menu) {
    drupal_add_js(drupal_get_path('theme', 'sitename_zen') . '/js/lib-conditional/toggle-menu.js', array('type' => 'file','weight' => 1055,'group' => JS_LIBRARY));
    drupal_add_css(drupal_get_path('theme','sitename_zen') . '/css/lib-conditional/toggle-menu.css');
  }

  drupal_add_library('sitename_zen', 'modernizr');
  drupal_add_library('sitename_zen', 'matchMedia_polyfill');
  drupal_add_library('sitename_zen', 'enquire');
  drupal_add_library('sitename_zen', 'responsive_js');
  drupal_add_library('sitename_zen', 'touch');
 ...
 */
function sitename_zen_preprocess_region(&$variables, $hook) {
 ...
 */
// function sitename_zen_preprocess_node(&$variables, $hook) {
 ...
 */
function sitename_zen_preprocess_block(&$variables, $hook) {
 ...
 */
function sitename_zen_block_list_alter(&$blocks) {

  // Hide the main content block on the front page if the theme settings are
  // configured that way.
  if (!theme_get_setting('sitename_zen_toggle_front_page_content') && drupal_is_front_page()) {
 ...
 *
 */
function sitename_zen_form_alter(&$form, &$form_state, $form_id) {
  // dpm ($form);
  // dpm ($form_id);
...
function sitename_zen_menu_alter(&$items) {
...
function sitename_zen_css_alter(&$css) {
